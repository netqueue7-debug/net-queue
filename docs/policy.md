...
- **Waiver requirement is per event/series, waiver content is per group.** A group can require its own (group-scoped) waiver on some events and not others. The group-waiver gate is checked alongside signup-open and capacity, inside the same RSVP transaction — never only in the UI. `waiverRequired` can only be set true on an event/series whose group actually has waiver content configured — there is nothing else to gate on otherwise. **There is no platform-level waiver** (removed 2026-09-03) — onboarding only requires a display name; the group waiver is the only tier.
...
- **Guest waivers never block anything.** Guest waivers are generated and sent, but an unsigned guest waiver does **not** block approval or attendance — admins collect signatures onsite. Show an "outstanding waiver" badge instead. (This is distinct from a group's own waiver, rule 6, which *does* gate a member's own RSVP when the event opts in. A guest signs the event's group's waiver when the group has one configured; when it doesn't, there is nothing to show or sign — no platform-level fallback.)
