...
| `docs/phase-0b-groups.md` | Groups, membership, join policy, per-group waivers — the multi-tenancy retrofit. |
...
7. **Group membership is the visibility boundary — with one override.** No active membership in a group means that group's events, RSVPs, and member list are invisible — enforced server-side on every read, not filtered client-side. Admin-ness is per group (a group admin's authority never crosses into a group they don't administer), **except** a platform admin (`users.role = admin`), who has full control over every group without needing a membership row — the rare ops tier, not how day-to-day group admins work. See `docs/architecture.md#groups--tenancy` and `docs/policy.md#6`.
...
- If a requirement seems ambiguous, check `docs/policy.md` first; those rules settle most disputes. If it's still ambiguous, ask rather than guess.
